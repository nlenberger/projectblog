...
First we must learn more about some specifict parameters. 

  * type- character string which specifies the trace type
  * color - values mapped to relevant 'fill-color'
  * stroke - similar to color, bu values are mapped to relevant 
      'stroke-color'
  * size - values mapped to relevant 'fill-size'

```{r}
plot_ly(babynames, x = ~year, y = ~prop, marker = list(size=10)) %>% 
